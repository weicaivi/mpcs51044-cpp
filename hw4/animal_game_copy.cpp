...
public:
    ...
    virtual unique_ptr<State> clone() = 0;
...
public:
    ...
    QuestionState(QuestionState const &other) :
        question(other.question), 
        yesNode(make_unique<Node>(*other.yesNode)),
        noNode(make_unique<Node>(*other.noNode)) {}
    QuestionState& operator=(QuestionState const &other) {
        yesNode = make_unique<Node>(*other.yesNode);
        noNode = make_unique<Node>(*other.noNode);
        return *this;
    }
    ...
    unique_ptr<State> clone() { return make_unique<QuestionState>(*this); }
private:
    string question;
    unique_ptr<Node> yesNode; // No need for smart ptrs here
    unique_ptr<Node> noNode;  // But only in this particular case
};
...
public:
    ...
    virtual void process(Node &node) override;
    unique_ptr<State> clone() { return make_unique<AnswerState>(*this); }
private:
    string animal;
};

Node::Node(string name) : state(make_unique<AnswerState>(name)) {}
Node::Node(Node const &other) : state(other.state->clone()) {}
